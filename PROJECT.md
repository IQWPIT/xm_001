...
- Browser result thumbnails use the same fixed square containment, so product images are always scaled into the card image area instead of changing card layout.
...
- Search supports `subject_search=true` from the browser UI, desktop app, and API. It crops the query image's likely foreground subject and masks a heuristic model face region before DINOv2 embedding, which reduces background, face, text, and ad-layout influence. Indexed product vectors remain unchanged.
